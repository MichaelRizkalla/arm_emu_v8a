...
namespace arm_emu {
    ...
    struct is_iteratable {
      private:
        ...
        constexpr static auto check(Tx*)
            -> decltype(std::begin(std::declval< Tx& >()) != std::end(std::declval< Tx& >()),
                        ++std::declval< decltype(std::begin(std::declval< Tx& >()))& >(),
                        --std::declval< decltype(std::begin(std::declval< Tx& >()))& >(),
                        ++std::declval< decltype(std::end(std::declval< Tx& >()))& >(),
                        --std::declval< decltype(std::end(std::declval< Tx& >()))& >(),
                        void(*std::begin(std::declval< Tx& >())), void(*std::end(std::declval< Tx& >())),
                        std::true_type {});
    ...
    struct is_indexable {
      private:
        template < typename Tx >
        constexpr static auto check(Tx*)
            -> decltype(std::declval< Tx& >()[std::declval< const typename Tx::size_type >()], std::true_type {});
    ...
    class ram_not_initialized : public std::exception {
        [[nodiscard]] virtual const char* what() const noexcept override {
            return "RAM block is not initialized";
        }
    };
