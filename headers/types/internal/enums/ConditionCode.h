...
namespace arm_emu {
	...
	enum class Condition : std::uint32_t {
		...
		NE = 0b0001,  // Z == 0				INT(not equal)						/ FP(not equal or unordered)
		CS = 0b0010,  // C == 1				INT(carry set)						/ FP(greater than, equal or unordered)
		HS = 0b0010,
		CC = 0b0011,  // C == 0				INT(carry clear)					/ FP(less than)
		LO = 0b0011,
		MI = 0b0100,  // N == 1				INT(minus, negative)				/ FP(less than)
		PL = 0b0101,  // N == 0				INT(plus, positive or zero)			/ FP(greater than, equal or unordered)
		VS = 0b0110,  // V == 1				INT(overflow)						/ FP(unordered)
		VC = 0b0111,  // V == 0				INT(no overflow)					/ FP(ordered)
		...
		GE = 0b1010,  // N == V				INT(signed greater than or equal)	/ FP(greater than or equal)
		LT = 0b1011,  // N != V				INT(signed less than)				/ FP(less than or unordered)
		...
		AL = 0b1110,  // Any				INT(Always)							/ FP(Always)
